	...
	const restartBtn = document.getElementById("restart"); /*кнопка рестарта*/	
    const canvasScore = document.getElementById('score');
    const contextScore = canvasScore.getContext('2d');	
    const divScore = document.getElementById('score2');
    ...
    /*Отправляетзапрос на добавление игрока в базу, в ответ получает его ID в базе*/
    ...
    /* маршрут и метод изменения score по ID игрока*/
    function writeScore(userid, score) {
		const request = new XMLHttpRequest();
		const url = "/ajax/ajax.writeScore.php?userid="+userid+"&score="+score;
		request.open('GET', url);
		request.setRequestHeader('Content-Type', 'application/x-www-form-url');
		request.send();
    }
	...
	function showScore() {
		...
		contextScore.font = '30px Courier New';

		/*в блоке "рейтинг игроков" высвечивается игроки с их количеством очков*/
		const request = new XMLHttpRequest();
		const url = "/ajax/ajax.getChampions.php";
		request.open('GET', url);
		request.setRequestHeader('Content-Type', 'application/x-www-form-url');
		request.addEventListener("readystatechange", () => {
			if (request.readyState === 4 && request.status === 200) {
			    ...
			    for (let k=1; k<11; k++) { /*цикл, проходящий по всем рядам оси y*/
				    if (document.querySelector (`[posX = "${k}"][posY = "${i}"]`).classList.contains('set')) { /*если какая то ячейка имеет класс set,то count повышается на единицу*/
					    ...
					    if (count == 10) { /* если count = 10, то ряд заполнен, класс set убирается*/
                            ...
                            writeScore(user_id, score); /* Вызывать fetch для изменения score каждый раз когда счёт меняется*/
	...
	restartBtn.addEventListener("click", (e) => { /*обработчик при клике на паузу*/
			location.reload()
	});	
